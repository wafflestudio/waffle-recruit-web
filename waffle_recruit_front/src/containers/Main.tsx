...
const Main: React.FC = () => {
  ...
  const match = useRouteMatch<{ prob_num: string }>();
  ...
  const problems = [
    '# 이미지 해독하기 :white_check_mark:\n' +
      ...
      '## 정답 예시' +
      'decoded: 111100011110100010111110100010\n' +
      '         100010100000100010000010110110\n' +
      '         100010100000010100000100101010\n' +
      '         111100011100001000001000101010\n' +
      '         101000000010001000010000100010\n' +
      '         100100000010001000100000100010\n' +
      '         100010111100001000111110100010' +
      'RSYZM' +
      '----',
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data.solved) {
          setProblem(problems[toNumber(match.params.prob_num) - 1]);
        } else {
          setProblem(problems[toNumber(match.params.prob_num) - 1].replace(':white_check_mark:', ''));
        }
      })
      .catch(() => {
  ...
  const markdownSolverStatus = '#### *지금까지 총 ' + solvers + '명이 성공했습니다 :fire:*\n';
